...
func (p *RemoteTokenProvider) Verify(ctx context.Context, answer service.AuthChallengeAnswer, request service.AuthChallengeInput) error {
	if strings.TrimSpace(answer.Token) == "" {
		return service.ErrAuthChallengeInvalid
	}
	form := url.Values{}
	form.Set("secret", p.secret)
	form.Set("response", answer.Token)
	if request.IP != "" {
