...
export function DependenciesSection({ canManage, dependencyAssets, groups, repositories, onEditAsset, stats }: DependenciesSectionProps) {
  ...
  const groupColumns: ColumnsType<PortGroupItem> = [
    ...
    },
    {
      ...
      render: (_, item) => (
        <Space size={[4, 4]} wrap>
          {item.repositoryLinks.length === 0 ? <Typography.Text type="secondary">-</Typography.Text> : null}
          {item.repositoryLinks.map((link) => (
  ...
  const repositoryColumns: ColumnsType<GitHubRepositoryItem> = [
    {
      title: "仓库",
      dataIndex: "fullName",
      render: (value, item) => <Typography.Link href={item.htmlUrl} target="_blank">{value}</Typography.Link>,
    },
    { title: "可见性", dataIndex: "visibility", width: 120, render: (value) => <Tag>{value}</Tag> },
  ...
  return (
    <Space direction="vertical" size={16} className="content-stack">
      <Row gutter={[12, 12]}>
        ...
        <Col xs={24} sm={6}>
          <WorkbenchPanel><Statistic title="GitHub 仓库" value={stats.repositories} /></WorkbenchPanel>
        </Col>
        <Col xs={24} sm={6}>
          <WorkbenchPanel><Statistic title="依赖资产" value={stats.dependencyAssets} /></WorkbenchPanel>
        </Col>
        <Col xs={24} sm={6}>
          <WorkbenchPanel><Statistic title="仓库关系" value={stats.repositoryLinks} /></WorkbenchPanel>
        </Col>
      </Row>
      <WorkbenchPanel title="GitHub 仓库">
        <Table<GitHubRepositoryItem>
          rowKey="id"
