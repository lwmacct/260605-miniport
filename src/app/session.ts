...
export async function loadSession(): Promise<SessionState> {
  try {
    const response = await fetch(authmeEndpoint("/session"), { credentials: "same-origin" });
    if (!response.ok) {
...
function isSessionState(value: unknown): value is Exclude<SessionState, { status: "unavailable" }> {
  if (!value || typeof value !== "object" || !("status" in value) || !("methods" in value) || !isMethods(value.methods)) {
    return false;
  }
  ...
  if (
    ...
    !("method" in value) ||
    !isMethodID(value.method) ||
    !("access" in value) ||
    (value.access !== "granted" && value.access !== "denied") ||
    !("identity" in value)
  ) {
    return false;
  }
...
function isMethods(value: unknown): value is AuthMethod[] {
  return Array.isArray(value) && value.length > 0 && value.every((method) => Boolean(
    ...
    "id" in method &&
    isMethodID(method.id) &&
    "flow" in method &&
    (method.flow === "redirect" || method.flow === "secret") &&
    "label" in method &&
